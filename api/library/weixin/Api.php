...
//        ::setFlash($flashKey, $state);
     ...
     * }';
     *
     * @param string $postData 需要发送的json格式数据
     * @return bool
     */
    public function sendTemplateMessage($postData)
    {
        ...
        $response = $this->curlPost(sprintf($url, $this->getAccessToken()), $postData);
        $data = json_decode($response, true);

        if (is_array($data) && array_key_exists('errcode', $data) && $data['errcode'] == '40001') {
           Yii::warning($response);
            $response = $this->curlPost(sprintf($url, $this->getAccessToken(false)), $postData);
            $data = json_decode($response, true);
        }
